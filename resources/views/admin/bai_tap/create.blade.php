...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Thêm bài tập mới</h1>
...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('admin.bai-tap.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('ten_bai_tap') is-invalid @enderror" 
                               id="ten_bai_tap" name="ten_bai_tap" value="{{ old('ten_bai_tap') }}" required>
                        @error('ten_bai_tap')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('nhom_co') is-invalid @enderror" id="nhom_co" name="nhom_co" required>
                                ...
                                <option value="nguc" {{ old('nhom_co') == 'nguc' ? 'selected' : '' }}>Ngực (Chest)</option>
                                <option value="lung" {{ old('nhom_co') == 'lung' ? 'selected' : '' }}>Lưng (Back)</option>
                                <option value="chan" {{ old('nhom_co') == 'chan' ? 'selected' : '' }}>Chân (Legs)</option>
                                <option value="vai" {{ old('nhom_co') == 'vai' ? 'selected' : '' }}>Vai (Shoulders)</option>
                                <option value="tay" {{ old('nhom_co') == 'tay' ? 'selected' : '' }}>Tay (Arms)</option>
                                <option value="bung" {{ old('nhom_co') == 'bung' ? 'selected' : '' }}>Bụng (Abs)</option>
                                <option value="cardio" {{ old('nhom_co') == 'cardio' ? 'selected' : '' }}>Cardio</option>
                            </select>
                            @error('nhom_co')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('do_kho') is-invalid @enderror" id="do_kho" name="do_kho" required>
                                <option value="de" {{ old('do_kho') == 'de' ? 'selected' : '' }}>Dễ</option>
                                <option value="trung_binh" {{ old('do_kho') == 'trung_binh' ? 'selected' : '' }}>Trung bình</option>
                                <option value="kho" {{ old('do_kho') == 'kho' ? 'selected' : '' }}>Khó</option>
                            </select>
                            @error('do_kho')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-circle me-2"></i>Lưu bài tập
                    </button>
                </form>
            </div>
        </div>
